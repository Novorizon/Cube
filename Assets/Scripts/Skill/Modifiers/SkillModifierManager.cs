...
namespace Game.Skill
{
    public sealed class SkillModifierManager
    {
        ...
        public IReadOnlyList<SkillModifierInstance> Modifiers
        {
            get
            {
                return modifiers;
        ...
        public void Update(float deltaTime)
        {
            for (int i = modifiers.Count - 1; i >= 0; i--)
            {
                ...
                if (instance.Parent == null || !instance.Parent.IsAlive)
                {
                    RemoveAt(i);
                    continue;
                }
        ...
        public bool AddModifier(ISkillUnit caster, ISkillUnit target, int modifierId, float duration)
        {
            ...
            if (!modifierMap.TryGetValue(modifierId, out SkillModifierData modifierData))
            {
                return false;
            }
        ...
        public float GetProperty(ISkillUnit unit, SkillModifierPropertyType propertyType)
        {
            ...
            for (int i = 0; i < modifiers.Count; i++)
            {
                ...
                if (instance.Data.PropertyType != propertyType)
                {
                    continue;
                }
