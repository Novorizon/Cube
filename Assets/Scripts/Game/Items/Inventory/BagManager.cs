...
namespace Game
{
    public sealed class BagManager
    {
        ...
        public const int TotalSlotCount = QuickSlotCount + InventorySlotCount;
        ...
        public void Initialize()
        {
            ...
            itemChangedSubscription = Messager.Instance.Subscribe<WorldMessageTopic, WorldItemChangedMessage>(
        ...
        public bool TryAddItem(int itemId, int count)
        {
            ...
            WorldItemManager.Instance.AddItem(itemId, count);
            NotifyFullRefresh();
            return true;
        }

        public bool TryAddItems(IReadOnlyList<WorldItem> items)
        {
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                WorldItemManager.Instance.AddItem(item.ItemId, item.Count);
            }
        ...
        public bool TryUseSlot(int slotIndex)
        {
            ...
            return ItemUseManager.Instance.TryUseItem(slot.ItemId);
        }
        ...
        public int GetSlotItemCount(int slotIndex)
        {
            return TryGetSlot(slotIndex, out BagSlot slot) && slot != null && !slot.IsEmpty
                ? WorldItemManager.Instance.GetCount(slot.ItemId)
                : 0;
        }
        ...
        private bool CanAddItems(IReadOnlyList<WorldItem> items)
        {
        ...
        {
            IReadOnlyDictionary<int, WorldItem> items = WorldItemManager.Instance.GetAllItems();
            if (items == null)
            {
                return;
            }

            foreach (KeyValuePair<int, WorldItem> pair in items)
            {
        ...
        private void RemoveZeroCountItems()
        {
            ...
            {
                BagSlot slot = slots[i];
                if (slot != null && !slot.IsEmpty && WorldItemManager.Instance.GetCount(slot.ItemId) <= 0)
                {
                    slot.Clear();
                }
            }
        ...
        private void OnWorldItemChanged(WorldItemChangedMessage message)
        {
            if (message == null)
            {
                return;
        ...
        private void NotifySlotChanged(int slotIndex)
        {
            ...
            Messager.Instance.Notify(WorldMessageTopic.BagChanged, new BagChangedMessage
            {
                ...
                Count = slot != null && !slot.IsEmpty ? WorldItemManager.Instance.GetCount(slot.ItemId) : 0,
                FullRefresh = false,
            });
        }
