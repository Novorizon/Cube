Shader "CubeTD/SoftLitWater"
{
    Properties
    {
        ...
        _FlowSpeed ("Flow Speed", Vector) = (0.04, 0.02, -0.025, 0.015)
        _RippleStrength ("Ripple Strength", Range(0, 0.35)) = 0.08
        _RippleScale ("Ripple Scale", Range(1, 40)) = 14
        _RippleSpeed ("Ripple Speed", Range(0, 6)) = 1.2
        _WaterBrightness ("Water Brightness", Range(0.5, 2)) = 1.18
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                half2 baseUv = input.baseUv + flowA;
                ...
                half waveB = sin((input.baseUv.x - input.baseUv.y) * (_RippleScale * 0.73h) - time * (_RippleSpeed * 1.27h));
                half ripple = (waveA + waveB) * 0.5h * _RippleStrength;

                half4 tex = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, baseUv + ripple * 0.025h) * _BaseColor;
                ...
                half detailWave = (detail - 0.5h) * _DetailStrength * 0.35h;
                tex.rgb *= max(0.5h, 1.0h + detailWave + ripple);
